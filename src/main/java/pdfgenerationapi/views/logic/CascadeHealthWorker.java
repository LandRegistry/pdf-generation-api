...
import pdfgenerationapi.utils.GenericWorker;
import pdfgenerationapi.utils.HealthUtils;
	...
	public GenericResponse<Map<String, Object>> process(Object body, Map<String, String> urlParams, Map<String, String[]> queryParams, Map<String, String> headers) {
		...
		for(Map.Entry<String, String> dependency : Config.DEPENDENCIES.entrySet()) {
			...
			if (s.startsWith("jdbc:")) {
				...
				/* More DB code to uncomment:

				Map<String, Object> db = new HashMap<String, Object>();
				db.put("name", dependency.getKey());
				
				try {
					HealthDAO.DB_TYPE databaseType = null;
					if (s.startsWith("jdbc:db2")) {
						databaseType = HealthDAO.DB_TYPE.DB2;
					} else {
						databaseType = HealthDAO.DB_TYPE.OTHER;
					}
					DateTime ts = healthDAO.getTimestamp(databaseType);
					db.put("current_timestamp", ts.toString());
					db.put("status", "OK");
				} catch (Exception e) { // Will also interecept the ApplicationException as we don't want it propagating
					db.put("status", "BAD");
					overall_status = 500;
				}
